...
export default function SampleTanStackQuery() {
  ...
  const {
    ...
    // 수동 페칭 메서드
    refetch: sampleMutationRefetch,
    // 초기 상태로 리셋 메서드
    reset: sampleMutationReset,
    // 페칭의 로딩 여부
    isLoading: sampleMutationIsLoading,
  } = useMutation({
    mutationKey: ["sample-mutation"],
    mutationFn: async ({ params }: SampleProps) => {
      const response = await postFakeData1(params);
      return response?.data;
    },
  });
  ...
  return (
    <>
      ...
      <button type="button" onClick={sampleMutationRefetch}>mutation fetch
      </button>
